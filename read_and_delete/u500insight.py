...
def processing_data(file, path_folder_to_read):
    ...
    data = U500Insight.ini_main(converted_data)
    datas = []
    for data in data:
        datas.append(data)
...
def write_file(path_folder_to_write, data):
    ...
    if len(data[0]) != 0:
        name_file = f'{year}-{month}-{day}_{hour}-{minute}-{second}_U500Insight_{data[0]}.txt'
    name_file = f'{year}-{month}-{day}_{hour}-{minute}-{second}_U500Insight.txt'
